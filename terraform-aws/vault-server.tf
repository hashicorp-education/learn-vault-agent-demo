...
resource "aws_instance" "vault-server" {
  ...
  user_data = templatefile("${path.module}/templates/userdata-vault-server.tftpl", { tpl_vault_zip_file = var.vault_zip_file, tpl_consul_zip_file = var.consul_zip_file, tpl_consul_dc = var.consul_dc, tpl_vault_service_name = "vault-${var.environment_name}", tpl_kms_key = aws_kms_key.vault.id, tpl_aws_region = var.aws_region, tpl_consul_bootstrap_expect = var.vault_server_count, account_id = data.aws_caller_identity.current.account_id, role_name = "${var.environment_name}-vault-client-role" })
