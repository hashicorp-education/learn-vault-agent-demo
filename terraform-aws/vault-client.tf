...
resource "aws_instance" "vault-client" {
  ...
  user_data = templatefile("${path.module}/templates/userdata-vault-client.tftpl", { tpl_vault_zip_file = var.vault_zip_file, tpl_consul_zip_file = var.consul_zip_file, tpl_consul_dc = var.consul_dc, tpl_vault_service_name = "vault-${var.environment_name}", tpl_vault_server_addr = aws_instance.vault-server[0].private_ip })
